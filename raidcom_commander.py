...
from action_select import ActionSelectForm, ActionSelectLogic
from helper_funcs import reset_ldev_provisioner_form, reset_ldev_terminator_form
...
class myEntryForm(nps.FormBaseNewWithMenus):
    ...
    def ldev_creator(self):
        form = self.parentApp.getForm('LDEV PROVISIONER')
        reset_ldev_provisioner_form(form)
        self.parentApp.switchForm('ACTION SELECT')

    def ldev_terminator(self):
        form = self.parentApp.getForm('LDEV TERMINATOR')
        Logo = ActionSelectLogic(form)
        Logo.update_config()
        reset_ldev_terminator_form(form)
...
class MyApplication(nps.NPSAppManaged):
    def onStart(self):
        ...
        self.addForm('ACTION SELECT', ActionSelectForm, name="ACTION SELECT")
